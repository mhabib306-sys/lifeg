      ...
      <div class="task-item compact-task group relative hover:bg-[var(--bg-secondary)]/50 rounded-lg transition cursor-pointer"
        ...
        <div class="flex items-center min-h-[32px] px-2 py-0.5">
          ...
          <div class="flex items-center gap-1.5 ml-2 flex-shrink-0 min-w-0 text-[11px]">
            ${area ? `<span class="text-[var(--text-muted)] truncate max-w-[70px] sm:max-w-[90px]">${escapeHtml(area.name)}${subcategory ? ' › ' + escapeHtml(subcategory.name) : ''}</span>` : ''}
            ${task.dueDate ? `<span class="flex-shrink-0 ${isOverdue ? 'font-medium' : isDueToday ? 'text-[var(--accent)] font-medium' : isDueSoon ? 'font-medium' : 'text-[var(--text-muted)]'}" style="${isOverdue ? 'color: var(--overdue-color)' : isDueSoon ? 'color: var(--flagged-color)' : ''}">${formatSmartDate(task.dueDate)}</span>` : ''}
